...
def simple_heuristic(file_path):
    ...
    for passenger_index, passenger in df.iterrows():
        ...
        isSecondClass = passenger['Pclass'] == 2
        isFirstClass = passenger['Pclass'] == 1
        isChild = passenger['Age'] < 14
        isYoungMother = (passenger['Age'] < 28) and isFemale and (not isChild)
        isFemaleAndElderly = isFemale and (passenger['Age'] > 50)
        isMaleAndElderly = isMale and (passenger['Age'] > 75)        
        
        if (isMale and (not isThirdClass) and isChild):
            predictions[passenger_id] = 1
        elif(isChild):
            predictions[passenger_id] = 1
        elif(isFemale and isFirstClass):
            predictions[passenger_id] = 1
        elif(isFemale and isSecondClass):
            predictions[passenger_id] = 1
        elif(isFemale and (isSecondClass or isThirdClass) and isYoungMother):
            predictions[passenger_id] = 1
        elif(isFemaleAndElderly or isMaleAndElderly):
            predictions[passenger_id] = 1
        else:
            predictions[passenger_id] = 0
